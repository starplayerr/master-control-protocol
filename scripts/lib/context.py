...
def generate_tree(repo_path: Path, max_depth: int = 3) -> str:
...
def gather_context(
    ...
    ctx = GatheredContext(repo_path=repo_path, tree=generate_tree(repo_path), files={})
    ...
    for fpath in unique_files:
        if _should_skip(fpath):
            ctx.excluded.append(str(fpath.relative_to(repo_path)))
            continue
